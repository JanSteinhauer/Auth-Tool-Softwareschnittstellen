...
/**
 This class represents the controller for "Profile 1" in the application.
 It extends the abstract class ProfileController and overrides its initialize method to initialize
 the profile settings for "Profile 1". It sets the profile and new profile settings to the ones
 stored in the database, sets the dark mode box, and sets the selected buttons according to the
 saved settings.
 */
public class Profile1Controller extends ProfileController{

    /**
     Initializes the controller by setting the profile and new profile settings to the ones
     stored in the database for "Profile 1", setting the dark mode box, and setting the selected
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        try {
            setProfile(getProfileDatabaseManager().getProfileByName("Profile 1").get());
