...
/**
 This class represents the controller for "Profile 2" in the application.
 It extends the abstract class ProfileController and overrides its initialize method to initialize
 the profile settings for "Profile 2". It sets the profile and new profile settings to the ones
 stored in the database, sets the dark mode box, and sets the selected buttons according to the
 saved settings.
 */
public class Profile2Controller extends ProfileController{
    /**
     Initializes the controller by setting the profile and new profile settings to the ones
     stored in the database for "Profile 2", setting the dark mode box, and setting the selected
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        try {
            setProfile(getProfileDatabaseManager().getProfileByName("Profile 2").get());
